...
const ScheduleConsultation: React.FC<Props> = ({
  ...
  const fetchBookedTimes = async (isoDate: string) => {
    ...
    try {
      const query = `query Consultations($date: String!) { consultations(where: { selectDate: { equals: $date } }) { preferredTime } }`;
      const res = await fetch(GRAPHQL_HTTP_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const times: string[] = consultations.map((c: any) => c.preferredTime).filter(Boolean);
  ...
  const fetchFullyBookedDates = async (monthDate?: Date | null) => {
    try {
      ...
      const query = `query ConsultationsByRange($start: String!, $end: String!) { consultations(where: { selectDate: { gte: $start, lte: $end } }) { selectDate preferredTime } }`;
      const res = await fetch(GRAPHQL_HTTP_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const json = await res.json();
      const rows: Array<{ selectDate?: string; preferredTime?: string }> = json?.data?.consultations || [];
